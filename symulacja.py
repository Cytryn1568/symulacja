...
class Pacjent:
    ...
    def __init__(self, x=0, y=0, v_x = 0, v_y = 0, status = 'zdrowy'):
        self.x = x
        self.y = y
        self.v_x = v_x
        self.v_y = v_y
    ...
    def set_status(self,wartosc):
        ...
        if wartosc not in ['zdrowy', 'chory', 'nosiciel']:
        ...
        for p in self._pacjenci:
            if p.status == 'zdrowy':
                for p2 in self._pacjenci:
                    zarazi = random.choices( [True, False], [25, 75] )[0]
                    dystans = sqrt( (p.x - p2.x)**2 + (p.y - p2.y)**2 )
                    if dystans>0 and dystans<15 and p2.status == 'chory' and zarazi:
                        p.set_status = 'chory'
                        break
